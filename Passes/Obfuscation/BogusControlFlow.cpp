...
PreservedAnalyses BogusControlFlowPass::run(Function& F, FunctionAnalysisManager& AM) {
    ...
    if (toObfuscate(flag, &F, "bcf")){
      ...
      if (BCFAnnotMD) {
        ...
        if (BCFAnnotMD->getNumOperands() != 1) {
          errs() << "Warning: BCF_annotations metadata has invalid operand count for function " << F.getName() << "\n";
          return PreservedAnalyses::all();
        }

        // 步骤3：将操作数安全转换为MDString（使用dyn_cast，避免类型不匹配崩溃）
        MDString *BCFAnnotStr = dyn_cast<MDString>(BCFAnnotMD->getOperand(0));
        if (!BCFAnnotStr) {
          errs() << "Warning: BCF_annotations metadata is not a valid MDString for function " << F.getName() << "\n";
          return PreservedAnalyses::all();
        }
