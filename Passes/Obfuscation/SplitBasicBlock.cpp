...
PreservedAnalyses SplitBasicBlockPass::run(Function& F, FunctionAnalysisManager& AM) {
    ...
    if (toObfuscate(flag, tmp, "split")){ // 判断什么函数需要开启混淆
      ...
      if (SPLITAnnotMD) {
        ...
        if (SPLITAnnotMD->getNumOperands() != 1) {
          errs() << "Warning: SPLIT_annotations metadata has invalid operand count for function " << (*tmp).getName() << "\n";
          return PreservedAnalyses::all();
        }

        // 步骤3：将操作数安全转换为MDString（使用dyn_cast，避免类型不匹配崩溃）
        MDString *SPLITAnnotStr = dyn_cast<MDString>(SPLITAnnotMD->getOperand(0));
        if (!SPLITAnnotStr) {
            errs() << "Warning: SPLIT_annotations metadata is not a valid MDString for function " << (*tmp).getName() << "\n";
            return PreservedAnalyses::all();
        }
